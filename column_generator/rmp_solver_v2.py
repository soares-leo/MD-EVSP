...
def build_rmp_model(
    ...
    use_artif_vars: bool = False  # MODIFIED SIGNATURE: Add new parameter with default
):
    ...
    # === CONDITIONAL LOGIC: Artificial Variables ===
    A = {}
    HIGH_COST = 100000
    if use_artif_vars:
        print("INFO: Building RMP with artificial variables for trip coverage.")
        for i, trip in enumerate(trips):
            A[i] = pl.LpVariable(f"Artif_{trip}", lowBound=0)
    # ===============================================
    ...
    # alpha constraints
    for i, trip in enumerate(trips):
        coverage_sum = pl.lpSum(
            coverage[(k,p,i)] * X[(k,p)]
            for k in range(len(grouped_routes))
            for p in range(len(grouped_routes[list(grouped_routes.keys())[k]]))
        )
        
        # === CONDITIONAL LOGIC: Add artificial variable to constraint ===
        if use_artif_vars:
            model += (coverage_sum + A[i] == 1, f"alpha_trip_{trip}")
        else:
            model += (coverage_sum == 1, f"alpha_trip_{trip}")
        # ===============================================================
    ...
    # objective function
    # === CONDITIONAL LOGIC: Add artificial variables to the objective ===
    objective_function = pl.lpSum(C[key] * X[key] for key in X)
    if use_artif_vars:
        objective_function += pl.lpSum(HIGH_COST * A[i] for i in A)
...
def run_rmp(
    ...
    timetables_csv: str = "initializer/files/timetables.csv",
    cplex_path: str = "/Applications/CPLEX_Studio2211/cplex/bin/arm64_osx/cplex",
    use_artif_vars: bool = False # MODIFIED SIGNATURE: Add new parameter
):
    model, X, C, coverage, trips, grouped = build_rmp_model(
        depots, initial_solution, timetables_csv,
        use_artif_vars=use_artif_vars # Pass the parameter down
    )
