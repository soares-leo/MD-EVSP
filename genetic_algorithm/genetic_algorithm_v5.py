...
class GeneticAlgorithm:
    ...
    def calculate_indicators(self, individual):
        """
        Calulate indicators and other useful data for further evaluation.
        Namely:
    ...
    def calculate_fitness_parameters(self, v_list, ru_list, s_list):
        min_v, avg_v = np.min(v_list), np.mean(v_list)
        min_ru, avg_ru = np.min(ru_list), np.mean(ru_list)  
        min_s, avg_s = np.min(s_list), np.mean(s_list)
        C_min = np.array([min_v, min_ru, min_s])
        C_avg = np.array([avg_v, avg_ru, avg_s])
        C_diff = [c_avg - c_min for c_avg, c_min in zip(C_avg, C_min)]
        phi_list = [0.6, 0.05, 0.4]
        PSI = np.array([math.log(phi) / c_diff if c_diff > 0 else 0 for phi, c_diff in zip(phi_list, C_diff)])
        
        return PSI, C_min, C_avg
    

    def evaluate_individual(self, individual, PSI, C_min):
        """
        ...
        v, r, u, s, cost, flat_trips = self.calculate_indicators(individual)

        C = np.array([v, r+u, s])

        # Calculate fitness
        exponent = PSI * (C - C_min)
        fitness = math.e ** np.sum(exponent)
    ...
    def run(self, verbose=True):
        ...
        v, r, u, s, cost, fitness = math.inf, math.inf, math.inf, math.inf, math.inf, math.inf
        
        while g <= self.max_gen:
            if verbose and g % 100 == 1:  # Print every 100 generations
                print(f"\nGENERATION {g} " + "-"*50)
                print(f"best v: {self.best_v}, best r: {self.best_r}, best u: {self.best_u}, best s: {self.best_s}, best cost: {self.best_cost:.4f}, best fitness: {self.best_fitness:.4f}")
                print(f"last v: {v}, last r: {r}, last u: {u}, last s: {s}, last cost: {cost:.4f}, last fitness: {fitness:.4f}")
                self.FITNESS_CACHE = {}

            v_list, r_list, u_list, ru_list, s_list, cost_list, fitness_list = [], [], [], [], [], [], []
            ...
            if g == 1:
                ...
                for ind in population:
                    v, r, u, s, cost, flat_trips = self.calculate_indicators(ind)
                    v_list.append(v)
                    r_list.append(r)
                    u_list.append(u)
                    ru_list.append(r+u)
                ...
                PSI, C_min, C_avg = self.calculate_fitness_parameters(v_list, ru_list, s_list)

                for ind in population:
                    eval_result = self.evaluate_individual(ind, PSI, C_min)
                    fitness = eval_result[0]
                    fitness_list.append(fitness)  # fitness
                ...
                self.best_v = v_list[best_idx]
                self.best_r = r_list[best_idx]
                self.best_u = u_list[best_idx]
                self.best_ru = ru_list[best_idx]
                self.best_s = s_list[best_idx]
            ...
            for ind in new_population:    
                eval_result = self.evaluate_individual(ind, PSI, C_min)
                fitness = eval_result[0]
                new_fitness_list.append(fitness)  # fitness
        ...
        return self.best_fitness, self.best_cost, selected_columns, PSI, C_min
...
def run_ga(columns, timetables_path, experiment_name, experiment_log_df, ga_params=None, verbose=True):
    ...
    if best_individual_tuple in ga.FITNESS_CACHE:
        ...
        eval_result = ga.evaluate_individual(ga.best_individual, PSI, C_min)
