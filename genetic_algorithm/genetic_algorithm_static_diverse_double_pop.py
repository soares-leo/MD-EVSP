...
class GeneticAlgorithm:
    ...
    def initialize_population(self):
        ...
        while len(initial_res_population) < self.pop_size:
            vehicles = rng_res.integers(self.L_min, self.L_max, endpoint=True)
            individual_array = np.random.choice(num_cols, size=vehicles, replace=False, p=weights_res)

            mask = np.full(num_cols, True, dtype=bool)
            mask[individual_array] = False
            weights_res = np.empty(num_cols)
            weights_res[individual_array] = 0.2 / len(individual_array)
            weights_res[mask] = 0.8 / len(weights_res[mask])

            initial_res_population.append(np.sort(individual_array))

        return initial_population, initial_res_population
    ...
    def compute_gene_freq(self, main_population):
        flattened_population = np.concatenate(main_population)
        uniq = np.unique_all(flattened_population)
        gene_freq = uniq.counts / len(main_population)
        return gene_freq, uniq.values
    
    def calculate_hamming_distance_single(self, indiv_a, indiv_b):
            """
            Calculates Hamming distance from lists of activated gene indices.

            This function is for a sparse chromosome representation where only the
            indices of "activated" (value of 1) genes are stored. The Hamming
            distance is the number of positions that differ, which is calculated
            as the size of the symmetric difference between the two sets of indices.

            Args:
                active_indices_a (list or np.ndarray): Sequence of indices where genes
                                                    are activated in the first chromosome.
                active_indices_b (list or np.ndarray): Sequence of indices where genes
                                                    are activated in the second chromosome.

            Returns:
                int: The Hamming distance between the two individuals.
            """
            # Convert the index lists to sets for a highly efficient comparison.
            set_a = set(indiv_a)
            set_b = set(indiv_b)

            # The symmetric difference gives you all the elements that are in one
            # set or the other, but NOT in both. This perfectly corresponds to
            # the gene positions that differ between the two chromosomes.
            differing_indices = set_a.symmetric_difference(set_b)

            # The count of these differing indices is the Hamming distance.
            return len(differing_indices)


    def hamming_distance_fitness(self, main_pop_gene_freq, main_pop_unique_values, res_pop_individual):
        #inner dist
        unique_in_indiv = np.where(np.isin(main_pop_unique_values, res_pop_individual, assume_unique=True))[0]
        ones = np.ones_like(unique_in_indiv)
        inner_dist = main_pop_gene_freq[unique_in_indiv] - ones

        #left dist
        mask = np.ones(len(main_pop_gene_freq), dtype=bool)
        mask[unique_in_indiv] = False
        left_dist = main_pop_gene_freq[mask]

        #outer dist
        indiv_notin_unique = ~np.isin(res_pop_individual, main_pop_unique_values, assume_unique=True)
        outer_dist = np.ones(len(res_pop_individual[indiv_notin_unique]))

        flat_scores = np.concatenate((inner_dist, left_dist, outer_dist))
        fitness = abs(np.sum(flat_scores))
        return fitness
    
    def evaluate_res_population(self, main_pop_gene_freq, main_pop_unique_values, res_population):
        res_pop_size = len(res_population)
        
        fitness_values = [
            self.hamming_distance_fitness(
                main_pop_gene_freq,
                main_pop_unique_values,
                indiv
            ) for indiv in res_population
        ]

        evaluations = [
            {
                'individual': res_population[i],
                'fitness': fitness_values[i]
            } for i in range(res_pop_size)
        ]
    ...
    def run(self, verbose=True):
        ...
        main_population, res_population = self.initialize_population()
        main_evaluations = self.evaluate_population_vectorized(main_population)
        all_evaluations = main_evaluations + self.evaluate_population_vectorized(res_population)
        main_pop_gene_freq, main_pop_unique_values = self.compute_gene_freq(main_population)

        
        if main_evaluations:
            initial_best_eval = max(all_evaluations, key=lambda x: x['fitness'])
        ...
        for g in range(1, self.max_gen + 1):
            main_fitness_list = [e['fitness'] for e in main_evaluations]
            ...
            new_main_population = [None] * self.pop_size
            new_res_population = [None] * self.pop_size
            
            for i in range(0, self.pop_size, 2):
                main_p1_idx, main_p2_idx  = self.spin_roulette(main_fitness_list)
                main_p1_fitness, main_p2_fitness = main_fitness_list[main_p1_idx], main_fitness_list[main_p2_idx]

                if main_p1_fitness > main_p2_fitness:
                    main_parent_1, main_parent_2 = main_population[main_p1_idx], main_population[main_p2_idx]
                else:
                    main_parent_1, main_parent_2 = main_population[main_p2_idx], main_population[main_p1_idx]      
                
                res_candidates_idx = np.random.choice(len(res_population), 4, replace=False)
                res_candidates = [res_population[i] for i in res_candidates_idx]

                res_candidates_hammings = [self.calculate_hamming_distance_single(res_candidates[i], main_parent_1) for i in range(4)]
                
                tournament_1_candidates = res_candidates[:2]
                tournament_2_candidates = res_candidates[2:]
                tournament_1_hammings = res_candidates_hammings[:2]
                tournament_2_hammings = res_candidates_hammings[2:]

                res_best_1_idx = np.argmin(tournament_1_hammings)
                res_best_2_idx = np.argmin(tournament_2_hammings)
                
                res_best_1 = tournament_1_candidates[res_best_1_idx]
                res_best_2 = tournament_2_candidates[res_best_2_idx]

                res_best_1_hamming = tournament_1_hammings[res_best_1_idx]
                res_best_2_hamming = tournament_2_hammings[res_best_2_idx]

                if res_best_1_hamming < res_best_2_hamming:
                    res_parent_1 = res_best_1
                    res_parent_2 = res_best_2
                else:
                    res_parent_1 = res_best_2
                    res_parent_2 = res_best_1

                if np.random.rand() <= self.crossover_p:
                
                    main_offspring_1, main_offspring_2 = self.make_crossover(main_parent_1, main_parent_2)
                    res_offspring_1, res_offspring_2 = self.make_crossover(res_parent_1, res_parent_2)
                    hybrid_offspring_1, hybrid_offspring_2 = self.make_crossover(main_parent_1, res_parent_1)

                    main_survivor_candidates = [main_offspring_1, main_offspring_2, hybrid_offspring_1, hybrid_offspring_2]
                    res_survivor_candidates = [res_offspring_1, res_offspring_2, hybrid_offspring_1, hybrid_offspring_2]

                    main_survivor_candidates_fitness = [self.evaluate_fitness(self.calculate_indicators(c)) for c in main_survivor_candidates]
                    res_survivor_candidates_fitness = [self.hamming_distance_fitness(main_pop_gene_freq, main_pop_unique_values, c) for c in res_survivor_candidates]

                    main_survivors_idx = np.argsort(main_survivor_candidates_fitness)[-2:]
                    res_survivors_idx = np.argsort(res_survivor_candidates_fitness)[-2:]

                    main_survivors = [main_survivor_candidates[i] for i in main_survivors_idx]
                    res_survivors = [res_survivor_candidates[i] for i in res_survivors_idx]
                
                else:

                    main_survivors = [main_parent_1, main_parent_2]
                    res_survivors = [res_parent_1, res_parent_2]

                new_main_population[i] = self.mutate(main_survivors[0])
                new_main_population[i+1] = self.mutate(main_survivors[1])
                new_res_population[i] = self.mutate(res_survivors[0])
                new_res_population[i+1] = self.mutate(res_survivors[1])
            
            
            all_new_population = new_main_population + new_res_population
            
            current_all_evaluations = self.evaluate_population_vectorized(all_new_population)

            # Elitism strategy
            current_all_fitness_list = [e['fitness'] for e in current_all_evaluations]
            best_current_gen_idx = np.argmax(current_all_fitness_list)
            worst_current_gen_idx = np.argmin(current_all_fitness_list)
            best_current_gen_eval = current_all_evaluations[best_current_gen_idx]
            ...
            sorted_indices = np.argsort(current_all_fitness_list)
            top_current_evals = [current_all_evaluations[i] for i in sorted_indices[-self.elite_indivs:]]
            ...
            if replacement_individual is not None:
                all_new_population[worst_current_gen_idx] = replacement_individual.copy()

                indicators = self.calculate_indicators(replacement_individual)
                fitness = self.evaluate_fitness(indicators)
                
                current_all_evaluations[worst_current_gen_idx] = {
            ...
            all_evaluations = current_all_evaluations
            ...
            cost_list = [e['cost'] for e in all_evaluations]
