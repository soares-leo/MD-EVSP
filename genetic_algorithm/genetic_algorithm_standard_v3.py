...
class GeneticAlgorithm:
    ...
    def initialize_population(self):
        ...
        while len(initial_population) < self.pop_size:
            ...
            individual_array = np.random.choice(num_cols, size=vehicles, replace=False)
    ...
    def evaluate_fitness(self, indicators):
        ...
        ru = indicators['r'] + indicators['u']
        v = indicators['v']
        s = indicators['s']
        denominator = (self.w1 * v + self.w2 * ru + self.w3 * s)
        if denominator == 0:
            return math.inf 
    ...
    def evaluate_population_vectorized(self, population):
        ...
        ru_values = np.zeros(pop_size, dtype=int)
        ...
        for i, individual in enumerate(population):
            ...
            ru_values[i] = indicators['r'] + indicators['u']
            s_values[i] = indicators['s']
            cost_values[i] = indicators['cost']
            
        indicator_matrix = np.vstack([v_values, ru_values, s_values]).T
        weights_vector = np.array([self.w1, self.w2, self.w3])
        fitness_values = self.w4 / np.dot(indicator_matrix, weights_vector)
