...
class GeneticAlgorithm:
    ...
    def __init__(self, columns, timetables_path, experiment_name, ga_params=None, depots={}):
        ...
        if 'fitness_components' in params and 'phi_values' in params:
            ...
            if translated_components:
                ...
                self.fitness_components = ['v', 'r', 'u', 's', 'd']
                self.phi_values = {'v': 0.01, 'r': 0.01, 'u': 0.8, 's': 0.2, 'd': 0.2}
        ...
        self.sigma5 = params.get('sigma5', 0.0)
        
        if self.sigma1 == 0.0 and self.sigma2 == 0.0 and self.sigma3 == 0.0 and self.sigma4 == 0.0 and self.sigma5 == 0.0:
    ...
    def _preprocess_columns(self):
        ...
        for i, key in enumerate(self.col_keys):
            ...
            self.preprocessed_data['depots'][i] = depot
    ...
    def initialize_population(self):
        ...
        while len(initial_population) < self.pop_size:
            ...
            weights[individual_array] = 0.05 / len(individual_array)
            weights[mask] = 0.95 / len(weights[mask])
    ...
    def calculate_indicators(self, individual_array):
        ...
        if v == 0:
            return {'v': 0, 'r': self.total_trips, 'u': self.total_trips, 's': 0, 'cost': math.inf, 'd': 0}
        ...
        for i in individual_array:
            depot = self.preprocessed_data['depots'][i]
            dep_counts[depot] = dep_counts.get(depot, 0) + 1

        for dep, count in dep_counts.items():
            dif = count - self.depots[dep]['capacity']
            if dif > 0:
                d += dif
        ...
        return {'v': v, 'r': r, 'u': u, 's': s, 'd': d, 'cost': cost}

    def evaluate_fitness(self, indicators):
        """Calculates an individual's fitness."""
        Sigma = np.array([self.sigma1, self.sigma2, self.sigma3, self.sigma4, self.sigma5]) * -1
        C = [indicators["v"], indicators["r"], indicators["u"], indicators["s"], indicators["d"]]
    ...
    def evaluate_population_vectorized(self, population):
        ...
        d_values = np.zeros(pop_size, dtype=int)
        ...
        for i, individual in enumerate(population):
            ...
            d_values[i] = indicators['d']
            cost_values[i] = indicators['cost']
            
        indicator_matrix = np.vstack([v_values, r_values, u_values, s_values, d_values]).T
        Sigma = np.array([self.sigma1, self.sigma2, self.sigma3, self.sigma4, self.sigma5]) * -1
        ...
        evaluations = [
            {
                ...
                'indicators': {
                    ...
                    's': s_values[i], 'd': d_values[i], 'cost': cost_values[i]
                },
    ...
    def log_ga_execution(self):
        ...
        v, r, u, s, d = bi.get('v', 0), bi.get('r', 0), bi.get('u', 0), bi.get('s', 0), bi.get('d', 0)
        
        metrics = ["ga_execution_time", "best_cost", "best_fitness", "best_v", "best_r", "best_u", "best_s", "best_d"]
        log_ids = [f"{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}{i}" for i in range(len(metrics))]
        values = [self.ga_execution_time, self.best_cost, self.best_fitness, v, r, u, s, d]
    ...
    def run(self, verbose=True):
        ...
        for g in range(1, self.max_gen + 1):
            ...
            if verbose and (g % 100 == 0 or g == 1):
                print(f"\nGENERATION {g} " + "-"*50)
                print(f"Best overall - Fitness: {self.best_fitness:.6f}, Cost: {self.best_cost:.2f}, v: {self.best_indicators.get('v')}, r: {self.best_indicators.get('r')}, u: {self.best_indicators.get('u')}, s: {self.best_indicators.get('s')}, d: {self.best_indicators.get('d')}")
            ...
            if verbose and (g % 100 == 0 or g == 1):
                ...
                print(f"Gen {g} best   - Fitness: {gen_best_eval['fitness']:.6f}, Cost: {gen_best_eval['cost']:.2f}, v: {gen_best_eval['indicators']['v']}, r: {gen_best_eval['indicators']['r']}, u: {gen_best_eval['indicators']['u']}, s: {gen_best_eval['indicators']['s']}, d: {gen_best_eval['indicators']['d']}")
        ...
        if verbose:
            ...
            print(f"Final best indicators - v: {self.best_indicators.get('v')}, r: {self.best_indicators.get('r')}, u: {self.best_indicators.get('u')}, s: {self.best_indicators.get('s')}, d: {self.best_indicators.get('d')}")
...
def run_ga(columns, timetables_path, experiment_name, ga_params=None, depots={}, verbose=True):
    """Run GA with provided columns and log results to CSV."""
    #profiler = cProfile.Profile()
    
    ga = GeneticAlgorithm(columns, timetables_path, experiment_name, ga_params, depots)
    ...
    if verbose:
        print(f"Summary: {v} vehicles, {r} repeated trips, {u} uncovered trips, {s} short routes, {d} violations of depots capacities.")
    
    return best_fitness, best_cost, v, r, u, s, d, selected_columns, ga_execution_log