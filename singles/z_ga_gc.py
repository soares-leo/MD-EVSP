...
from genetic_algorithm.genetic_algorithm_dynamic import run_ga
import pandas as pd
import datetime
import os
import json
...
# Fixed parameters for the post-column generation GA
POST_CG_GA_PARAMS =    {
        "max_gen": 10000,
        "crossover_p": 0.8,
        "mutation_p": 0.01,
...
def save_final_results(selected_columns, experiment_info, file_path):
    """
    Save the final selected columns and experiment metadata to a JSON file.
    
    Args:
        selected_columns: Dictionary of selected routes
        experiment_info: Dictionary with experiment metadata
        file_path: Path to save the file
    """
    # Convert route data to a serializable format
    serializable_columns = {}
    for route_name, route_data in selected_columns.items():
        serializable_route = {}
        for key, value in route_data.items():
            if isinstance(value, pd.Timestamp):
                serializable_route[key] = value.isoformat()
            elif isinstance(value, (list, tuple)):
                serializable_route[key] = list(value)
            else:
                serializable_route[key] = value
        serializable_columns[route_name] = serializable_route
    
    # Create the final output structure
    output_data = {
        "timestamp": datetime.datetime.now().isoformat(),
        "experiment_info": experiment_info,
        "selected_columns": serializable_columns,
...
def run_experiments():
    ...
    os.makedirs("logs/pre_cg_ga", exist_ok=True)
    os.makedirs("logs/post_cg_ga", exist_ok=True)
            ...
            for tmax_list, rand_ear, rand_tmax, tmax_min_list, inverse_list in zip(
                ...
                for _i, params in enumerate(GA_PARAMS):
                    ...
                    if type == "GACG":
                        ...
                        # Create a custom log dataframe for pre-CG GA
                        pre_cg_ga_log_df = log_df.copy()
                        
                        try:
                            best_fitness, best_cost, selected_columns, pre_cg_ga_log_df = run_ga(
                                unique_routes, timetables, _i, pre_cg_ga_log_df, tmax_list, params
                            )
                            print(f"✓ Pre-CG GA completed. Best cost: {best_cost}, Selected columns: {len(selected_columns)}")
                            
                            # Save pre-CG GA log
                            pre_cg_log_path = f"logs/pre_cg_ga/set_{i+1}_params_{_i+1}.csv"
                            pre_cg_ga_log_df.to_csv(pre_cg_log_path, index=False)
                            print(f"✓ Pre-CG GA log saved to {pre_cg_log_path}")
                            
                        except Exception as e:
                            print(f"⚠ Error in Pre-CG GA: {e}")
                            with open("error.txt", "a") as err:
                                err.write(f"Error occurred in Pre-CG GA with parameters: {params}\n")
                        ...
                        # STEP 6: Run GA on the generated columns (Post-Column Generation)
                        print("=" * 80)
                        print("STEP 6: Running GA (Post-CG) on generated columns...")
                        print("=" * 80)
                        print(f"Running Post-CG GA with fixed parameters:")
                        print(f"  - Population size: {POST_CG_GA_PARAMS['pop_size']}")
                        print(f"  - Max generations: {POST_CG_GA_PARAMS['max_gen']}")
                        print(f"  - Mutation rate: {POST_CG_GA_PARAMS['mutation_p']}")
                        print(f"  - Crossover rate: {POST_CG_GA_PARAMS['crossover_p']}")
                        print(f"  - Input columns: {len(columns)}")
                        print()
                        
                        # Create a custom log dataframe for post-CG GA
                        post_cg_ga_log_df = pd.DataFrame(columns=["experiment_name", "step", "metric", "value"])
                        
                        # Add experiment metadata to log
                        post_cg_ga_log_df = pd.concat([
                            post_cg_ga_log_df,
                            pd.DataFrame([{
                                "experiment_name": f"post_cg_ga_{set_name}_{_i}",
                                "step": "initialization",
                        ...
                        try:
                            # Run GA with fixed parameters on the generated columns
                            post_cg_best_fitness, post_cg_best_cost, post_cg_selected_columns, post_cg_ga_log_df = run_ga(
                                columns,  # Use the columns generated by column generation
                                timetables,
                                f"post_cg_{_i}",  # Different experiment ID
                                post_cg_ga_log_df,
                                tmax_list,
                                POST_CG_GA_PARAMS  # Use fixed parameters
                            )
                            
                            print(f"✓ Post-CG GA completed.")
                            print(f"  - Best cost: {post_cg_best_cost}")
                            print(f"  - Selected columns: {len(post_cg_selected_columns)}")
                            print(f"  - Reduction: {len(columns)} → {len(post_cg_selected_columns)} columns")
                            print()
                            
                            # Save post-CG GA log
                            post_cg_log_path = f"logs/post_cg_ga/set_{i+1}_params_{_i+1}.csv"
                            post_cg_ga_log_df.to_csv(post_cg_log_path, index=False)
                            print(f"✓ Post-CG GA log saved to {post_cg_log_path}")
                            ...
                            experiment_info = {
                                ...
                                "post_cg_ga_params": POST_CG_GA_PARAMS,
                                ...
                                "statistics": {
                                    "initial_routes_for_ga": len(unique_routes),
                                    "independent_solution_routes": len(independent_solution),
                                    "pre_cg_selected_columns": len(selected_columns),
                                    "combined_solution_routes": len(combined_selected_columns),
                                    "columns_generated": len(columns),
                                    "post_cg_selected_columns": len(post_cg_selected_columns),
                                    "pre_cg_best_cost": best_cost,
                                    "post_cg_best_cost": post_cg_best_cost,
                                    "column_generation_optimal": optimal
                                }
                            }
                            
                            # Save final results
                            results_file = f"results/final_set_{i+1}_params_{_i+1}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                            save_final_results(post_cg_selected_columns, experiment_info, results_file)
                            ...
                            summary_df = pd.DataFrame([{
                                ...
                                "set_number": i + 1,
                                "param_set": _i + 1,
                                "initial_routes": len(unique_routes),
                                "pre_cg_selected": len(selected_columns),
                                "columns_generated": len(columns),
                                "post_cg_selected": len(post_cg_selected_columns),
                                "pre_cg_cost": best_cost,
                                "post_cg_cost": post_cg_best_cost,
                                "reduction_ratio": len(post_cg_selected_columns) / len(columns)
                            }])
                            ...
                            print(f"\n{'='*80}")
                            print(f"FATAL ERROR IN POST-CG GA")
                            print(f"{'='*80}")
                            print(f"Parameters used: {POST_CG_GA_PARAMS}")
                            print(f"Number of columns: {len(columns)}")
                            print(f"Experiment ID: post_cg_{_i}")
                            print(f"{'='*80}")
                            raise  # This will re-raise the original exception with full traceback
    ...
    # Save main experiment log
    log_df.to_csv("experiments_log.csv", index=False)
    print("\n✓ Main experiment log saved to experiments_log.csv")
    
    # Print summary statistics if available
    if os.path.exists("results/experiment_summary.csv"):
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("STARTING EXPERIMENT RUNNER WITH POST-COLUMN GENERATION GA")
    print("=" * 80)
    print(f"Timestamp: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Number of experiment sets: {NUM_OF_SETS}")
    print(f"Types: {TYPES}")
    print(f"Post-CG GA will use fixed parameters:")
    for key, value in POST_CG_GA_PARAMS.items():
        print(f"  - {key}: {value}")
    print("=" * 80 + "\n")
    ...
    print("  - logs/pre_cg_ga/    : Logs from GA before column generation")
    print("  - logs/post_cg_ga/   : Logs from GA after column generation")
    print("  - results/           : Final selected columns and experiment summaries")