...
def run_experiments():
    ...
    for i in range(NUM_OF_SETS):
        ...
        timetables = 'generated_timetables/combined_timetable_20250905_013355.csv'
            ...
            for tmax_list, rand_ear, rand_tmax, tmax_min_list, inverse_list in zip(
                ...
                for _i, params in enumerate(GA_PARAMS):
                    ...
                    if type == "GACG":
                        ...
                        print("Deduplicating routes after CG...")
                        dedup_staging = []
                        final_routes = {}
                        route_counter = 0
                
                        for k, v in columns_light.items():
                            if v["Path"] in dedup_staging:
                                continue
                            dedup_staging.append(v["Path"])
                            final_routes[f"Route_{route_counter}"] = v
                            route_counter += 1
                        ...
                        columns_data = {
                            'columns': final_routes,
                            'metadata': {
                                'set_number': i + 1,
                                'param_set': _i + 1,
                                'timetables_path': timetables,
                                'optimal_value': optimal,
                                'num_columns': len(final_routes),
                                'timestamp': timestamp,
